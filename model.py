...
class TBML_DetectionModel(nn.Module):
    ...
    def forward(self, kyc_x, edge_index, swift_edge_attr, seq_data, trade_features):
        ...
        # enriched_nodes shape: [num_nodes, hidden_dim]
        enriched_nodes = F.relu(self.gat(kyc_x, edge_index, edge_attr=swift_edge_attr))
        
        # We take the mean of the sender and receiver node embeddings to represent the transaction "context"
        ...
        # graph_context (64) + seq_data (1) = 65 features
        # We need to repeat the graph context for every step in the sequence
        ...
        # 4. Feed the combined (65-dim) vector to LSTM
